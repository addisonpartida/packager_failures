#this file handles the imported json data from api query and grabs 
#desired information from logs including date, req_id, and error message
# into a dataframe to then be loaded into a csv for export to bq   
...
def categorize_packager_errors(error_message):
    ...
    error_patterns = {
        ...
        'Empty Metadata':{
            ...
            r'.* Written metadata file should contain data.*'
        ...
        'Config not found': [
            r'.*java.io.FileNotFoundException:.*does not exist in cloud storage.*'
...
def parse_json(results):
    ...
    for entry in results:
        ...
        # categorize the error messages with helper function
        error_type = categorize_packager_errors(stream_error)

        # Check if the request ID already exists in the dictionary
        if stream_reqid in table_dictionary:
            existing_epoch_times = [entry["epoch_time"] for entry in table_dictionary[stream_reqid]]
            # Check if the current entry's epoch time is within 10 seconds of any existing epoch time
            if any(abs(epoch_time - existing_epoch_time) <= 10 for existing_epoch_time in existing_epoch_times):
                continue 

            # If not within 10 seconds, add the entry to the list for the request ID
            table_dictionary[stream_reqid].append({
                "date": stream_time,
    ...
    for req_id, entries in table_dictionary.items():
